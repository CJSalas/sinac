<?php
    ...
    function getInstitutions($page){
        ...
        foreach ($array as $data){
            $html = $html."<tr>
                <td></td>
                <td id=".$data["idInstitucion"]." presupuesto=".$data["presupuesto_anno"].">"
                    .utf8_decode($data["nombre_institucion"])."
                    <input id='presupuesto_anno' name='presupuesto' type='hidden' value=".$data["presupuesto_anno"].">
                    <input id='id_sector' name='idSector' type='hidden' value=".$data["id_Sector"].">
                    <input id='funcion_institucion' name='funcion' type='hidden' value='".$data["funcion_institucion"]."'>
                    <input id='objetivo_redd' name='objetivo' type='hidden' value='".$data["objetivo_redd"]."'>
                </td>
                <td>
    ...
    function getInstitutionsSectors(){
        ...
        $array = $sectores->searchSectors($page, $record_per_page);
        $html = "";
        $html = "<select class='selection-2 select2-hidden-accessible' id='txtSectoresInstitucion' name='service' tabindex='-1' aria-hidden='true'>";
        foreach ($array as $data){
            $html = $html."<option value=".$data["idSector"].">".utf8_decode($data["nombre_sector"])."</option>";
        }
        $html = $html."</select>";
        /*<select class="selection-2 select2-hidden-accessible" id="txtSectoresInstitucion" name="service" tabindex="-1" aria-hidden="true">
